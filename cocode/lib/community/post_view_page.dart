...
class _PostViewPageState extends State<PostViewPage> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: user.getPost(postIndex),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          var data = snapshot.data;
          return Scaffold(
            appBar: AppBar(
              title: Text(data['title'], style: TextStyle(color: black)),
                  ...
                  Expanded(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        ...
                        Text('시간 - ${data['upload']}'),
                        SingleChildScrollView(child: Text(data['content'])),
                      ],
                    ),
                  ),
                  Expanded(
                    ...
                    child: Form(
                      ...
                      child: Column(
                        children: <Widget>[
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  if (_key.currentState.validate()) {
                                    ...
                                    myController.clear();
                                    String url = user.mainUrl + '/post/comment';
                          ...
                          FutureBuilder(
                            future: user.getCommnet(postIndex),
                            builder: (context, snapshot) {
                              if (snapshot.hasData) {
                                return Expanded(
  ...
  isMyComment(String name, int index) {
    if (user.name == name) {
      return IconButton(
        ...
        onPressed: () async {
          String url = user.mainUrl + '/post/deleteComment/$index';
